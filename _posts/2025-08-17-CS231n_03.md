...
다음으로 신경망들의 가중치를 어떻게 초기화해야하는지를 살펴봅시다. 그냥 아무렇게나 초기화하면 되는거 아닌가 싶잖아요? 그런데 아무렇게 초기화하면 깊은 신경망에서는 큰 문제가 발생합니다. 예를 들어 tanh를 활성화함수로 사용하는 신경망에서 어떻게 초기화를 해볼지 생각해봅시다. 가장 먼저 드는 생각은 모든 가중치를 0으로 초기화하는 방법입니다. 처음에 0으로 초기화를 시켜놓으면 학습이 진행됨에 따라 점점 적절한 파라미터 값으로 도달하게 될 것이다 라는 순진한 발상이죠. 모든 값을 0으로 0이 아니더라도 동일한 값으로 초기화를 하게 되면 발생하는 치명적인 문제점이 있는데, 그건 바로 대칭성이 깨지지 않는다는 점입니다.
...
MLP 상의 모든 뉴런들은 기본적으로 이전 레이어의 모든 출력을 입력으로 받기 때문에 대칭적인 구조를 가집니다. 그런데 모든 레이어의 파라미터를 0이라는 동일한 값으로 맞추게 되면 예측값은 0이 될 것이고 예측값과 실제 값 사이에는 차이가 있을 테니까 손실이 존재하고 그로부터 기울기가 발생할 텐데요, 문제는 모든 뉴런이 동일한 값을 내뱉기 때문에 동일한 기울기를 받게 되고 동일한 만큼의 파라미터 업데이트가 수행되고, 이게 아무리 계속 반복되어도 한 레이어의 모든 뉴런이 동일한 업데이트를 수행하게 되고 대칭성이 깨지지 않고 유지됩니다. 우리가 다양한 뉴런을 사용하는 이유가 각각의 뉴런이 서로 다른 특징에 집중해 좀 더 다양하고 많은 특징들을 사용하게 하기 위함인데, 이렇게 모두 동일하게 학습이 수행되면 당연히 성능이 좋지 않게 되겠죠. 
...
그래서 모든 값을 0으로 딱 초기화시키는게 아니라 평균이 0인 표준편차가 꽤 작은 정규분포에서 파라미터를 랜덤으로 결정합니다. 이렇게 하면 처음부터 파라미터 값들에 차이가 있기 때문에 각기 다른 방향으로 파라미터가 업데이트되고 뉴런이 각각 다른 특징에 집중하도록 분화됩니다. 꽤나 괜찮은 방식인데 레이어를 깊게 쌓으면 또 문제가 발생합니다.
...
위와 같이 표준편차를 0.01인 정규분포에서 파라미터를 설정하면 절댓값이 0.02 이내의 값들이 레이어를 거듭하면서 계속 곱해지게 될 것입니다. 이렇게 작은 숫자를 가지는 가중치가 계속 곱해지면, 당연히 각 레이어의 출력인 Activation의 분포의 분산은 점점 줄어들게 됩니다. 이게 어떤 문제를 유발할까요?
...
Backpropagation을 통해 기울기를 구해나갈 때, 이 후 레이어의 가중치가 Chain rule 상에 지속적으로 곱해지거든요? 근데 그 값은 
