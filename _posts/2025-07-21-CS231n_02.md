...
위  코드는 CNN 레이어를 3층 쌓고, 뒤로 FC 레이어를 2층 쌓아 만든 모델입니다. 보통 CNN을 사용해서 Neural Network를 구성할 때에는 CNN을 앞단에 여러 층 배치한 다음, 뒤에 FC 레이어를 사용해 레이블 개수만큼 원소를 가지는 벡터로 반환하도록 만듭니다. 앞의 CNN 레이어들은 이미지에서 특징들을 뽑아 추상화시키는 역할을 한다고 해서 **Feature Extractor(특징 추출기)**라고 하구요, 뒤의 MLP 레이어들은 추상화시킨 정보를 바탕으로 분류 작업을 수행하는 역할을 한다고 해서 **Classifier(분류기)**라고 합니다.

총 10개의 레이블로 구성된 이미지를 분류해야하는 태스크를 학습시켜봤는데요, 약 78%정도의 성능을 확인 가능합니다. MLP로만 구성되는 모델을 동일한 파라미터 수를 가지도록 만들어 학습시켜보면 훨씬 더 낮은 성능이 나오는데, 확실히 공간적 근접성 정보를 잘 활용하는 CNN 아키텍쳐가 이미지를 분류할 때 아주 유리하다는 것을 알 수 있죠.
