...
$$
\mathbf a = \text{activation function}(\mathbf h)
$$

$$
\mathbf{y} = W_{2}\,\mathbf{a} + \mathbf{b}_{2}
$$



이렇게 Activation Function을 사용해 선형 변환을 거친 출력을 휘어주면 여러 계층이 하나의 선형 함수로 더 이상 통합되지 못하고 각각이 결정 평면을 조절하는 역할을 수행하게 됩니다. 결과적으로는 복잡하게 구부러진 결정 평면을 만들 수 있게 되는 것입니다. 이 새로운 구조에서 선형 계층을 통과하고 비선형 함수를 통과하기 까지의 과정을 묶어서 **Perceptron** 또는 **Neuron**이라고 부릅니다.  
...
이 코드가 퍼셉트론을 2층으로 쌓아 만든 모델이구요, 모델 학습 결과로 얻은 결정 평면을 아래와 같습니다.
