...
각 레지스터는 특정 용도로 사용되기 위해 규약이 정해져 있으나, 이는 엄격한 규칙이 아닌 권장 사항에 불과합니다. 이러한 규약은 어셈블리 코드의 이해와 호환성을 높이기 위한 목적으로 존재합니다. 예를 들어, `rax`는 주로 함수의 반환 값이나 산술 연산에 사용되고, `rbx`는 메모리 주소 저장에, `rcx`는 반복문 카운팅에 주로 사용됩니다. 함수의 매개변수는 `rdi`, `rsi`, `rdx`, `rcx`, `r9`, `r8` 순으로 저장되며, `rsp`는 스택 포인터로, 스택의 시작을 가리키는데 사용됩니다.
